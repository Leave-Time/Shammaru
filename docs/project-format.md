...
Answers can be generated by a reference solution (`answer.mode: reference_solution`) or loaded from committed static files (`answer.mode: static`). A problem-level template overrides the project-level template, which overrides the built-in template.
